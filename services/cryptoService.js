...
export const getMarketData = async (domesticCurrency) => {
  try {
    const response = await axios.get(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${domesticCurrency}&order=market_cap_desc&per_page=100&page=1&sparkline=true&price_change_percentage=7d`
    );
    const { data } = response;
...
export const getUserMarketData = async (assetIds, domesticCurrency) => {
  let assetCalls;
  if (assetIds.length > 1) {
    assetCalls = assetIds.join('%2C%20');
  } else {
    assetCalls = assetIds[0];
  }
